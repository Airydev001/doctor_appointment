...
<div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-96 border-none rounded-xl text-zinc-600 text-sm shadow-lg'>
  ...
  <p>Please {state ==="Sign Up" ? "sign up" : "log in"} to book apppointment</p>
{
...
{
  state === "Sign Up"
  ? <p>Already have an account? <span onClick={()=>setState('Login')} className="to-blue-300 underline cursor-pointer">Login here</span> </p>
  :  <p>Create an new account? <span onClick={()=>setState('Sign Up')} className='to-blue-400 underline cursor-pointer'>click here</span> </p>
}
</div>
    </form>
  )
